...
## Are strongest Z500 days linked to drought? Are they hotter with drought?

# Identify top 10% of z500 days
z_high = []
z_limit = z500_reg.collapsed('time', iris.analysis.PERCENTILE, percent=[10, 90]).data[1]
for each in z500_reg.slices(['time'])[:3]:
    print(each)

    if each[0].data > z_limit:
        print(each[0].data)

iris.Constraint()
# T for same days
# PS for preceeding 2 and 4 weeks
# EP for preceeding 2 and 4 weeks
...
for n, each in enumerate(yr):
    ...
    prev2week_cons = iris.Constraint(day_of_year=lambda cell: z_day[n][0]-14 <= cell <= z_day[n][0])
    prev4week_cons = iris.Constraint(day_of_year=lambda cell: z_day[n][0]-21 <= cell <= z_day[n][0])
    ...
    P2_data.append(np.mean(ps_reg.extract(yr_cons & prev4week_cons).data))
    E2_data.append(np.mean(ep_reg.extract(yr_cons & prev4week_cons).data))
...
zip_object = zip(P2_data, E2_data)
D2_data = []
for l1, l2 in zip_object:
    D2_data.append(l1-l2)
...
# scatter plots
sc = plt.scatter(D1_data, Z_data, c=T_data, vmin=30, vmax=40, cmap=cm,  marker="o", alpha=0.7)
